...
class FileServiceTest extends CloudFileBaseTest
{
    public function testGetUploadTemporaryCredential()
    {
        ...
        $res = $filesystem->getUploadTemporaryCredential($credentialPolicy, $this->getOptions());
    ...
    public function testSimpleUpload()
    {
        ...
        $filesystem->uploadByCredential($uploadFile, $credentialPolicy, $this->getOptions());
    ...
    public function testDuplicate()
    {
        ...
        $path = $filesystem->duplicate('easy-file/test.txt', 'easy-file/test-copy.txt', $this->getOptions());
    ...
    public function testAppendUpload()
    {
        ...
        $realPath = __DIR__ . '/../test.txt';
        $appendUploadFile = new AppendUploadFile($realPath, 0, 'easy-file/1');
        $filesystem->appendUploadByCredential($appendUploadFile, $credentialPolicy, [
            'token' => '',
        ]);
        $this->assertTrue(true);
    }

    private function getFilesystem(string $storage = 'file_service_test'): FilesystemProxy
    {
    ...
    {
        // 单测前需要改一下这个token，该包暂不支持 ci 自动化测试，没有做 mock，没有做动态 token
        return [
            'token' => 'xxx',
